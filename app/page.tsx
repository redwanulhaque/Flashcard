        ...
        <div className="card mb-4">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={async () => {
                if (
                  !confirm("Delete this study tool and all its flashcards?")
                )
                  return;
                try {
                  const res = await fetch(
                    `/api/study-tools?toolId=${selectedTool.id}`,
                    {
                      method: "DELETE",
                    }
                  );
...
function Flashcard({
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    try {
      const res = await fetch(`/api/study-tools?flashcardId=${id}`, {
        method: "DELETE",
      });
